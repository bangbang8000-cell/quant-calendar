...
| 策略研究菜单 | 研究菜单（量化研究/回测/市场复盘）显示开关（异动扫描入口已删除，后端 scan_engine 保留离线扫描能力） |
...
quant-calendar/
├── README.md
├── docs/                        ← 当前文档（PRD / DEV-PLAN / TEST-PLAN / EVAL，统一 v5.x 命名；V5-ASSESSMENT、DESIGN-SYSTEM；旧版本归档于 docs/archive/ 本地保留）
...
│   ├── portfolio_builder.py     ← 组合构建（合成因子选 topN / 权重 / 净值）
│   ├── api/v1/portfolio.py      ← 模拟组合/持仓路由（持仓数据落 db.py）
...
> **版本编号说明（2026-09 起，务必先读）**
>
> 1. 本项目曾从 v5.4.3 跳到 v6.0.0、一路做到 v6.9.6（**该线全程未打过任何 tag**）。现已**整体并回 5.X**，此后长期在 5.X 上推进。
> 2. **当前发布版本 = v5.5.0**（`APP_VERSION`）。这次并入的落地改动：默认导航形态改为顶部二级标签（toptab）、`element-plus.css` 补 `?v=` 版本号根治下拉框文字/箭头换行。
> 3. 为保留那条开发线**内部的里程碑顺序**，配套的 PRD / DEV-PLAN / TEST-PLAN / EVAL 文档按 **6.N → 5.(N+5)** 平移编号：6.0→5.5、6.1→5.6、6.2→5.7、6.3→5.8、6.5→5.10、6.6→5.11、6.9→5.14。
>    例如 `docs/PRD-v5.14.4.md` 就是原 PRD-v6.9.4，`docs/TEST-PLAN-v5.11-5.14.md` 就是原 TEST-PLAN-v6.6-6.9。
> 4. 因此**「发布版本号 v5.5.0」与「文档里程碑编号 5.5–5.14」用途不同**：前者是运行时版本（`/api/health` 返回的那个），后者只是文档命名。下表按平移后的编号倒序列出各里程碑。
> 5. 文档正文里的 `TC-6.9.3.xx` 测试用例号、`tests/test_v69x3_*.py` 等代码路径**保持原样**，因为它们与仓库中真实存在的测试文件一一对应，改名会失去可追溯性。

| 版本 | 日期 | 变更 |
|------|------|------|
| v5.14.5 | 2026-09 | 用户菜单功能修复：修改密码/退出登录/重新运行初始化向导点击生效（EP 消息服务显式挂载 + 菜单项调用修复 + 用户信息响应式解包）/ 一二级菜单选中未选中均不加粗 / 深色股票弹窗 K 线周期 tab 文字色修复（选中主题色底+深字，未选中白字） |
| v5.14.4 | 2026-09 | EP 样式表对齐 2.14.5 根治下拉按钮换行 / 三处股票列表移除共识进度条 / 主题深浅色模式按钮高亮 / 深色输入框适配 / 策略研究空数据可诊断 / 导航形态面板精简 / 宽度类收敛 / 页面组件静态预注册根治研究·系统等页空白 |
| v5.14.3 | 2026-09 | 12 项 UI 优化 — 股票列表信息密度 / 下拉规范 / 搜索框右移 / 铃铛面板 / 主题按钮 / 日历工具栏 / 短线复盘提速 / R1 清理 / sxsc 测试修复 / 功能配置精简 / 菜单重排 |
| v5.14.2 | 2026-09 | 菜单/工作区字体优化 + 主题颜色系统动态联动 |
| v5.14.1 | 2026-09 | 菜单体系精简：删除异动扫描 / 系统状态一级菜单 / 暗色蓝色统一 / TOP5 策略中文名 / ops 菜单 i18n 与入口 / 拆分状态概览与配置保存 / 修复 ops 执行看板无数据 |
| v5.14.0 | 2026-09 | AI 评估校准分析 |
| v5.13.0 | 2026-09 | 行情日线批量缓存 |
| v5.12.0 | 2026-09 | 易用性与操作便捷性 |
| v5.11.1 | 2026-09 | 信息架构重构 — 菜单层重组 + 系统配置方案 A |
| v5.10.1 | 2026-09 | 策略总览概览页去除半透明毛玻璃 |
| v5.10.0 | 2026-09 | 全站 Lucide 图标 + 弹窗圆角 + 搜索框窄宽 + 短线复盘概览容错 + 龙虎榜降级透出 |
| v5.8.0 | 2026-09 | 导航形态配置化（subnav/tree/toptab）+ 顶部栏容器化/页签圆角矩形 + 评估分析命名修复 + 功能配置页入口 |
| v5.7.0 | 2026-09 | 导航与界面细节收口 |
| v5.6.0 | 2026-09 | 三栏式布局与视觉系统统一：中栏二级常驻 / 动态页签 / 信息密度 / 移动端抽屉 / 页签关闭修复 |
| v5.5.1 | 2026-09 | 设计系统落地（DS-5.5）：导航与视觉打磨 / 主题适配（dark-pro 金色 scale）/ 基础组件统一层 / KPI Dashboard / 动效收尾 |
| v5.5.0 | 2026-09 | 导航与菜单系统重构：组件 SFC 化 + 设计系统地基 + 金色默认主题 |
...
- ✅ 开放 API 扩容（v4.7 已交付：复盘 / 胜率 / 因子 IC 等端点，/api/v2 前缀）
- ✅ 更多语言包（v4.7 已交付 en / ja / ko / zh-TW，共 5 语）
