...
| 评估胜率追踪 | 评估命中率统计（总体/分模型/分评级），决策复盘；V5.9.2 独立「评估分析」子页 + 持久缓存(TTL 6h, 数据源故障降级) |
...
| 权限与协作 (V5.8) | RBAC 2.0(权限点注册表+5 内置角色+自定义角色+数据范围) / 共享自选组/评估备注/组合可见性 / 协作并发 RLock 原子写 |
| 数据质量 (V5.1) | 质量规则引擎(缺数/异常/复权/停牌/对齐, 质量分 A-D) / PIT 防前视治理 / 幸存者偏差治理 / 数据血缘与刷新批次(batch_id 追溯) / 数据字典(系统配置子页) |
| 通知中心 (V5.4) | 通知通道抽象(飞书/Webhook/邮件/钉钉/企微/Telegram+退避重试) / 事件引擎 2.0(去重+投递日志) / 自定义预警规则(价格突破/涨跌幅/量比/入池) / 通知中心页 |
...
| 性能优化 | 首屏 +42%、K 线 5000 点降采样、历史分页懒加载；V5.7 两级缓存(L1 内存+L2 磁盘) / 异步任务队列 / LTTB 降采样+年视图预算守护 / 前端虚拟滚动+性能门禁 |
| 引导与效率 (V5.6) | 新手引导任务流 / 空态/错误态体系化 / 命令面板+全局快捷键(Ctrl+K/F5/Ctrl+B/Ctrl+J) / 信息密度选项 |
...
quant-calendar/
├── README.md
├── docs/                        ← 当前文档（PRD/DEV-PLAN/TEST-PLAN v5.0-5.9、V5-ASSESSMENT、DESIGN-SYSTEM；旧版本归档于 docs/archive/ 本地保留）
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v5.9.2 | 2026-09 | 四项需求：执行看板内容空修复(execRateClass 未导出→渲染崩) / 策略回测移入策略研究 / sub.datadict 菜单中文(语言包补齐+t() 兜底治本) / 评估历史拆成评估历史+评估分析(命中率持久缓存 TTL 6h+数据源故障降级) / 2260 测试全绿 |
| v5.9.1 | 2026-09 | 修复日/周/月/年视图无股票列表(calendar-page state.viewNote 模板引用未定义 + onboarding UMD 双写构建 bug) + dist 重建 |
| v5.9.0 | 2026-09 | 架构现代化与工程化：ai_evaluator/scheduler/data_sources/merrill_clock 拆分子包(Mixin 薄壳) / schema 迁移框架 backend/migrations / 一键升级/回滚脚本 / 观测性 2.0(SLO 指标+结构化日志) / 契约/属性测试 / 2251 测试全绿 |
| v5.8.0 | 2026-09 | 多用户协作与开放平台 2.0：RBAC 2.0(权限点注册表+5 内置角色+自定义角色+数据范围) / 协作(共享自选组/评估备注/组合可见性/RLock 原子写) / API v3(分页/过滤/错误码信封) / Python SDK / 插件 SDK 2.0 / +130 用例 |
| v5.7.0 | 2026-09 | 性能与规模化：两级缓存(L1 内存 TTL+L2 磁盘原子写) / 异步任务队列(jobs 后台 worker+协作取消+重试+进度) / 全市场分块+LTTB 降采样+年视图预算守护 / 前端虚拟滚动+性能门禁入 CI / +86 用例 |
| v5.6.0 | 2026-09 | 用户体验与引导：新手引导任务流(5 步状态机+覆盖层) / 空态/错误态体系化(5 语标准文案) / 命令面板+全局快捷键(Ctrl+K/F5/Ctrl+B/Ctrl+J) / 信息密度选项(comfortable/compact) / 可访问性 2.0(WCAG AA) / +80 用例 |
| v5.5.0 | 2026-09 | 报表中心与可视化：报表模板化(6 类区块) / PDF/Excel 零依赖导出 / 报表订阅+投递闭环 / 图表语义配色令牌 / 首页今日要点聚合卡 |
| v5.4.0 | 2026-09 | 实时行情与事件通知中心：通知通道抽象(飞书/Webhook/邮件/钉钉/企微/Telegram+退避重试) / 事件引擎 2.0(事件→订阅→通道+去重) / 自定义预警规则(价格突破/涨跌幅/量比/入池) / WS 行情 2.0(增量推送+心跳) / 通知中心页 |
| v5.3.0 | 2026-09 | 风险与组合管理：组合风险指标(波动/VaR/CVaR/回撤/夏普/Sortino/Calmar/Beta) / 仓位建议(Kelly/波动率目标/风险平价) / 风控规则引擎(集中度/止损止盈/回撤熔断) / 风险预警事件总线 |
| v5.2.0 | 2026-09 | 专业回测与研究台：成本模型 2.0(印花税/佣金/滑点/冲击,可插拔) / 基准对比(沪深300/500/1000+IR/alpha/beta) / walk-forward 滚动回测(无前视+OOS 稳定) / 参数稳定性分析(过拟合诊断) / 绩效归因(Brinson+因子) / 回测报告导出(CSV/HTML) |
| v5.1.0 | 2026-09 | 数据中台与准确性：质量规则引擎(5 规则+质量分 A-D) / PIT 防前视治理(评估/回测/因子/日历四路径) / 幸存者偏差治理(PIT 退市改名注册表) / 数据血缘与刷新批次(batch_id 追溯) / 数据字典(系统配置子页) |
