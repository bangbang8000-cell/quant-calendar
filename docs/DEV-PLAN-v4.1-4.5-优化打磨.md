...
- **产品基线**：master 187a232（V4.7.3，登录修复 + 版本 bump）| **配套**：PRD-v4.1-4.5-优化打磨.md | TEST-PLAN-v4.1-4.5-优化打磨.md
...
| V4.7 | 数据真实化+并发安全 | 6 | 2-3 天 | ✅ v4.7.2 已发布 |
| **合计** | | **57** | **14-21 天** | **全部完成** |
...
### 4.7 V4.7 数据真实化+并发安全 (V4.0 主线延续)
| # | 任务 | 主要文件 | 验证方式 | 状态 |
|---|------|----------|----------|:--:|
| 7.1 | 引擎全市场批量取数(universe=all, 按交易日批量接口) | data_sources.py, data_portal.py | 持仓 8 列→5557 列 | ✅ |
| 7.2 | 选股池真实轮动修复(18 号后不变化) | strategy_governance.py, views_aggregator.py | 日视图 8→88 只 | ✅ |
| 7.3 | 年视图性能 32 倍提速(8.7s→0.27s) | views_aggregator.py | 年视图 <0.5s | ✅ |
| 7.4 | 引擎 run-once 异步化(to_thread 不阻塞事件循环) | scheduler.py, strategy_research.py | run-once 期间 health 2.7ms | ✅ |
| 7.5 | 持仓文件原子写入(tmp+os.replace) + save_state 部分更新 | data_parser.py, strategy_governance.py | 并发读无半截 | ✅ |
| 7.6 | 数据源 Token 掩码编辑锁+长度校验 + V4.7.2 发布 | data_sources.py, secret_utils.py | 掩码不被当真值写入 | ✅ |

## 5. 后续维护与待办 (V4.8+)

### 5.1 当前状态

V4.1-V4.7 全部完成后，产品已进入**维护模式**。代码 TODO 仅 1 处。

> ⚠️ **2026-08-25 实测修正**：V4.7.2 的 CI（Python 3.13）此前因 `backend/ai_models.py:77` 缺 `List` 导入而收集失败（10 个测试文件 ERROR）；本机 Python 3.14 惰性注解（PEP 649）未暴露此问题。已修复（`from typing import Dict` → `from typing import Dict, List`）并验证 3.13/3.14 收集均无错误，CI 转绿。

### 5.2 待办任务

| # | 任务 | 类型 | 主要文件 | 估时 | 优先级 |
|---|------|:----:|----------|:--:|:-----:|
| 8.1 | ~~Python 3.13(CI) 收集修复 — `backend/ai_models.py` 补 `List` 导入~~ **✅ 2026-08-25 已修复** | 修复 | backend/ai_models.py | 0.5 天 | **P0 已完成** |
| 8.2 | 因子 IC 接入真实数据 — `factor_ic.py` TODO 落地 | 功能 | backend/factor_ic.py, api/v1/market.py, stock-detail.js | 1 天 | P2 |
| 8.3 | 完整链路验证脚本 — WS/复盘/事件/数据源全链路 | 工具 | scripts/verify_chain.sh | 1-2 天 | P2 |
| 8.4 | 开放 API 扩容 — 复盘/胜率/IC 端点纳入开放 API | 功能 | backend/api/v1/openapi.py, api_keys.py | 1-2 天 | P3 |
| 8.5 | 更多语言包 — 日/韩/繁中 | i18n | frontend/js/locales/ | 1-2 天 | P3 |
| 8.6 | `except: pass` 治理 — 后端 bare except 改 logging | 治理 | backend/*.py | 1 天 | P3 |

### 5.3 建议版本规划

```
V4.8.0  [补齐]  因子 IC 真实数据 + 链路验证脚本（P0 收集修复已于 2026-08-25 单独完成）
V4.9.0  [开放]  开放 API 扩容 + except 治理
