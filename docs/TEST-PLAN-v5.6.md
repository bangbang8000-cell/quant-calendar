# 量化日历 V5.6 测试计划（TEST-PLAN 6.1 · 三栏式布局与视觉系统统一）

- **文档版本**：v1.0（待评审）
- **日期**：2026-09-09
- **产品基线**：V5.5（refactor/ui-v6 分支）
- **配套**：PRD-v5.6.md | DEV-PLAN-v5.6.md
...
- `#page` / `#page/sub` / `#page/invalid-sub` / 空 hash → 断言状态回写行为与 V5.5 一致（无效子页忽略）。
- **状态**：复用/扩展现有。
...
- 折叠态 + 移动端 TabBar / 抽屉。
- **比对阈值**：与基线像素差 ≤ 0.5%（沿用 V5.5 视觉回归流程，`tests/e2e/visual_regression.py` 扩展）。
