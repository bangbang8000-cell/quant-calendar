# TEST-PLAN v5.11：界面美术打磨收尾

> 版本：v5.11（草稿，待评审）
> 状态：待用户评审
> 前置：PRD-v5.11 / DEV-PLAN-v5.11
...
| L4 回归 | 既有 V5.10/V6.x 门禁 + 全量关键集 | 无新增回归 |

> 已知 pre-existing 失败：`test_transition_tokens.py::test_transition_uses_tokens_or_standard`（V5.5 handover 有意设计冲突，**不属于本期范围**，回归记录中注明即可）。
...
| ID | 用例 | 预期 |
|---|---|---|
| TC-6.6.4.1 | V5.10 门禁（test_v65_m0/m2）+ V5.8/V5.7 门禁 | 通过（除 pre-existing `test_transition_tokens`） |
