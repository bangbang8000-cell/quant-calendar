# 量化日历 V5.6 产品需求文档（PRD 6.1 · 三栏式布局与视觉系统统一）

- **文档版本**：v1.0（待评审）
- **日期**：2026-09-09
- **产品基线**：V5.5（导航与菜单系统重构已落地）
- **配套**：DEV-PLAN-v5.6.md | TEST-PLAN-v5.6.md
- **需求来源**：`Navigation-System-Evaluation-v2.md`（用户 6+1 项方向）与 `Navigation-System-Handover-v1.md`（第十六章实施指南）
- **状态**：待用户评审
- **范围约束**：沿用 V5.5 分支纪律，本地开发，不推送主线；版本纪律 APP_VERSION 由 `backend/main_new.py` 单一来源 bump 至 6.1.0
...
V5.5 把导航从"页面内 Tab + 侧边栏混用"收敛成"一级侧边栏 + 二级 SubNav"两个 SFC 组件，并建立了 `--qc-*` 设计 token 与金色默认主题，工程地基已经扎实。但视觉与交互的**一致性**仍是短板，具体表现为六类分散问题：
...
> P3 项（F10）在验收期确认，若影响进度可顺延至 V5.7，不阻塞本期主线。
...
| F3 | 页面组件无 `.page-header` 内部标题；二级切换只存在于中栏；各子页数据加载行为与 V5.5 一致 |
...
完整任务拆解与验收细节见 DEV-PLAN-v5.6.md，测试用例见 TEST-PLAN-v5.6.md。
