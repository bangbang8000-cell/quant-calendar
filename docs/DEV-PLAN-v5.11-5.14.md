# DEV-PLAN v5.11.X–v5.14.X：存量功能优化打磨阶段开发计划

> 版本：v1.0（整合版，待终审）
> 状态：待用户终审授权
> 创建日期：2026-09-11
> 需求来源：《PRD-v5.11-5.14.md》（唯一需求来源，含已确认决策 D7 方案 A、D8 开放 API 暂不拆分）
> 配套文档：《TEST-PLAN-v5.11-5.14.md》（测试计划，独立）
...
| 已知 pre-existing 失败 | `test_transition_tokens.py::test_transition_uses_tokens_or_standard`（V5.5 有意设计冲突，**不属于本阶段范围**，回归记录注明即可） |
...
### 2.1 v5.11.X —— 美术收尾 + 信息架构重构

> 目标：完成 V5.11 既定美术收尾（F-6.6.1~6.6.6），落地菜单层重构（F-6.6.7）与系统配置整改方案 A（F-6.6.8）。

#### M-6.6.0 美术收尾（承接 PRD-v5.11，定版 6.6.0）

> V5.11.0 已按 PRD-v5.11 规划，本里程碑承接其全部任务清单（M0 快速项/M1 emoji 清理/M2 token 迁移/M3 回归）。此处列收尾确认项。
...
- 重新引入动态页签（V5.9 用户拍板移除）。
- **拆分开放 API 为独立子页（决策 D8）**；**采用方案 B 折叠 health/schedule/guard（决策 D7 已否决）**。
- 不做未在 PRD 列出的新功能域。

---

*文档结束。待用户终审授权后按 §2 分版本启动开发；测试按《TEST-PLAN-v5.11-5.14.md》执行。*
