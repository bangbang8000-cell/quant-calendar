# PRD v5.11：界面美术打磨收尾（基于 V5.7 美术评估报告）

> 版本：v5.11（草稿，待评审）
> 状态：待用户评审
> 需求来源：《V5.7 美术与 UI 综合评估报告》（Navigation-System-Evaluation-v4.md）、《导航与菜单系统编码交接文档 v1.4 第二十章》（Navigation-System-Handover-v1.md）
...
V5.10 已落地部分 V5.7 美术评估整改项（主要页面 emoji→Lucide、AppIcon 白名单扩展、搜索框缩窄、state-panel 支持 Lucide、策略总览半透明去除）。实测基线显示仍有以下残留，构成 V5.11 范围：
...
V5.10 已清理 system/strategies/shortterm/弹窗 + 语言包，但以下文件仍有装饰性 emoji：
...
- 按 V5.7 评估报告第八节「emoji 替换速查表」映射（⭐→star、🤖→bot、📈→trending-up、📊→bar-chart-3、🟢🟡🔴→彩色 dot + 文字、✓✗→check/x 等）。
...
**目标**：迁移为语义 token 类。推荐新增（对齐 V5.7 评估 8.2 节）：
