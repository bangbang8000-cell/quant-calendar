# 量化日历 V5.7 产品需求文档（PRD 6.2 · 导航与界面细节收口）

- **文档版本**：v1.0（待评审）
- **日期**：2026-09-09
- **产品基线**：V5.6（三栏式布局与视觉系统统一已落地）
- **需求来源**：`Navigation-System-Evaluation-v3.md`（用户更新后的设计文档）
- **配套**：DEV-PLAN-v5.7.md | TEST-PLAN-v5.7.md
- **状态**：待用户评审
...
V5.6 已把导航收敛为「左侧一级 + 中栏二级 + 工作区 + 动态页签」的三栏架构，主题/图标/Logo 完成统一。但对照 v3 评估报告，仍有 4 处明确的细节缺口与若干可打磨点：
...
- **股票列表组件化**：在 V5.6 纯 CSS 类（`.qc-stock-list`）基础上，进一步抽象 `StockList.vue` 组件，统一各页面列表渲染入口。
...
- **移动端（<768px）**：
  ...
  - 搜索框收缩（见 F3），为页签腾出空间。
- **交互**：激活页签即当前位置（语义取代面包屑）；点击页签 `activateTab`，关闭按钮 `closeTab`（复用 V5.6 状态机）。
...
- **交互**：聚焦态保留 `--qc-ring` 描边（沿用 V5.5 覆盖）。
...
- **组件**：`frontend/src/components/common/StockList.vue`，基于 V5.6 `.qc-stock-list/.qc-stock-row` 样式封装，渲染结构：排名（可选）/ 代码 + 状态徽标 / 名称 / 策略标签 / `#extra` 插槽 / `#actions` 插槽。
...
- 页签状态机复用 V5.6 `tabs-core.js` 与 `qcState` 的 openTab/closeTab/activateTab。
...
完整任务拆解与验收细节见 DEV-PLAN-v5.7.md，测试用例见 TEST-PLAN-v5.7.md。
