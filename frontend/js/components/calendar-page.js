...
(function () {
  ...
  window.__quantComponents.CalendarPage = {
    ...
    setup() {
      ...
      // V5.3.14 (T-SP.P0.1): 多策略并集/交集对比
      const compareVisible = ref(false);
      const compareLoading = ref(false);
      const compareError = ref('');
      const compareData = ref(null);
      const comparePairs = ref([]);
      function _dispName(sid) {
        const m = { multifactor: '多因子', industry_rotation: '行业轮动', index_enhance: '指数增强', money_flow: '资金流' };
        return m[sid] || sid;
      }
      async function openStrategyCompare() {
        ...
        comparePairs.value = [];
        try {
          const res = await fetch('/api/calendar/' + state.selectedDate.value + '/compare');
          ...
          for (const k of Object.keys(comp)) {
            if (k === 'all_intersection') continue;
            const v = comp[k] || {};
            const parts = k.split('_vs_');
            pairs.push({
              label: _dispName(parts[0]) + ' ↔ ' + _dispName(parts[1]),
              interCount: v.intersection_count || 0,
              inter: (v.intersection || []).join(', '),
              onlyS1Count: v.only_s1_count || 0,
              onlyS1: (v.only_s1 || []).join(', '),
              onlyS2Count: v.only_s2_count || 0,
              onlyS2: (v.only_s2 || []).join(', '),
            });
          }
          comparePairs.value = pairs;
        } catch (e) {
          compareError.value = String(e && e.message ? e.message : e);
        } finally {
