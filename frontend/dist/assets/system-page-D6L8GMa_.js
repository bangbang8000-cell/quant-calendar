...
<!-- 数据新鲜度 -->
    ...
    `,setup(){const t=Z("qcState");if(!t)return{};Vue.watch(()=>t.currentSubPage&&t.currentSubPage.value,e=>{e==="autoeval"&&t.loadAiVendors&&t.loadAiVendors(),e==="datadict"&&U(),e==="health"&&N()});const ee=[45,220,0,140,270,320],j={45:"金色",220:"蓝色",0:"红色",140:"绿色",270:"紫色",320:"粉色"},se=Vue.computed(()=>{const e=window.__quantModules&&window.__quantModules.preferences;return e&&e.getPreference&&e.getPreference("theme")||"system"}),p=Vue.ref(45);(function(){const e=window.__quantModules&&window.__quantModules.preferences,s=e&&e.getPreference&&e.getPreference("theme_hue");s!=null&&s!==""&&(p.value=parseInt(s,10))})();function ae(e){t.changeThemeMode&&t.changeThemeMode(e)}function te(e){p.value=parseInt(e,10),t.changeThemeHue&&t.changeThemeHue(p.value)}function le(e){return"hsl("+e+", 75%, 42%)"}function ie(e){return j[e]||"自定义 "+e}const I=Vue.ref([]),g=Vue.ref(""),H=Vue.ref("read"),d=Vue.ref(""),f=Vue.ref(!1),c=()=>window.__quantModules&&window.__quantModules.core||{},b=Vue.ref([]),y=Vue.ref(!1);async function ce(){y.value=!0;try{const e=await fetch("/api/audit/logs?limit=20",{headers:c().authHeaders?c().authHeaders():{}}).then(function(s){if(!s.ok)throw new Error("HTTP "+s.status);return s.json()});b.value=e&&e.logs||[]}catch(e){console.error("[system] 审计加载失败:",e),b.value=[]}finally{y.value=!1}}const w=Vue.ref(!1),x=Vue.ref(null),L=Vue.ref(null),F=Vue.ref([]),ne=Vue.ref(null);function re(e){return e==="completed"?"完成":e==="running"?"运行中":e==="pending"?"排队中":e==="cancelled"?"已取消":"失败"}async function m(){try{const s=await(await fetch("/api/jobs?limit=20")).json();s&&s.success&&(F.value=s.data&&s.data.tasks||[])}catch(e){console.warn("[system] 加载任务队列失败:",e)}}async function de(e){try{await fetch("/api/jobs/"+e+"/cancel",{method:"POST"}),ElementPlus.ElMessage.success("已请求取消任务"),m()}catch(s){console.warn("[system] 取消任务失败:",s)}}function oe(){m(),ne.value=window.setInterval(m,15e3)}const v=Vue.ref({items:[]}),k=Vue.ref([]),K=Vue.ref(null),O=Vue.ref({data_sources:[],alerts:[]}),ve=function(){return c().authHeaders?c().authHeaders():{}},u=function(e){return fetch(e,{headers:ve()}).then(function(s){if(!s.ok)throw new Error("HTTP "+s.status);return s.json()})};async function N(){w.value=!0,x.value=null;try{const[e,s,a,r]=await Promise.all([u("/api/reliability/freshness"),u("/api/reliability/heal-history?limit=20"),u("/api/reliability/startup-report"),u("/api/reliability/source-health")]);v.value=e&&e.data||{items:[]},k.value=s&&s.data||[],K.value=a&&a.data||null,O.value=r||{data_sources:[],alerts:[]},L.value=new Date().toLocaleTimeString()}catch(e){console.warn("[health] 加载失败:",e),x.value="健康数据加载失败: "+(e.message||""),v.value={items:[]},k.value=[]}finally{w.value=!1}}const _=Vue.ref(!1),C=Vue.ref(""),A=Vue.ref(""),z=Vue.ref({fields:[]});async function U(){_.value=!0,C.value="";try{const e="/api/data-dict"+(A.value?"?category="+A.value:""),s=await u(e);z.value=s&&s.data||{fields:[]}}catch(e){console.warn("[dict] 加载失败:",e),C.value="数据字典加载失败: "+(e.message||""),z.value={fields:[]}}finally{_.value=!1}}function ue(e){return{fresh:"var(--color-success)",stale:"var(--color-danger)",missing:"var(--text-tertiary)",unknown:"var(--color-warning)"}[e]||"var(--text-secondary)"}function pe(e){return{fresh:"正常",stale:"过期",missing:"缺失",unknown:"未知"}[e]||e}const me=Vue.computed(()=>(v.value?v.value.items||[]:[]).filter(s=>s.status==="stale"||s.status==="missing").length),q=Vue.ref("rules"),B=Vue.ref([]),G=Vue.ref([]),W=Vue.ref([]),i=Vue.ref(!1),R=Vue.ref(""),M=Vue.ref("price_above"),P=Vue.ref(""),o=Vue.ref(!1),J=Vue.ref(60),l=Vue.ref("");function he(e){return{price_above:"价格突破",price_below:"价格跌破",pct_change:"涨跌幅超",volume_surge:"量比异动",new_pool:"入池"}[e]||e}async function h(){i.value=!0;try{const e=await(await fetch("/api/alerts/rules")).json();B.value=e&&e.rules||[]}catch(e){l.value="规则加载失败: "+e}finally{i.value=!1}}async function Q(){i.value=!0;try{const e=await(await fetch("/api/alerts/history?limit=50")).json();G.value=e&&e.history||[]}catch(e){l.value="历史加载失败: "+e}finally{i.value=!1}}async function Y(){i.value=!0;try{const e=await(await fetch("/api/alerts/channels")).json(),s=await(await fetch("/api/alerts/silence")).json();W.value=e&&e.channels||[],o.value=!!(s&&s.silenced)}catch(e){l.value="通道状态加载失败: "+e}finally{i.value=!1}}function ge(e){q.value=e,e==="rules"?h():e==="history"?Q():Y()}async function fe(){const e=R.value.trim();if(!e){l.value="请填写股票代码";return}i.value=!0;try{const s={stock_code:e,rule_type:M.value};if(M.value!=="new_pool"){const r=Number(P.value);if(isNaN(r)){l.value="阈值必须为数值";return}s.threshold=r}const a=await(await fetch("/api/alerts/rules",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(s)})).json();a&&a.rule?(l.value="规则已添加",R.value="",P.value="",h()):l.value=a&&a.detail||"添加失败"}catch(s){l.value="添加失败: "+s}finally{i.value=!1}}async function be(e){try{await fetch("/api/alerts/rules/"+e.id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:!e.enabled})}),e.enabled=!e.enabled}catch(s){l.value="切换失败: "+s}}async function ye(e){try{const s=await(await fetch("/api/alerts/rules/"+e.id,{method:"DELETE"})).json();s&&s.success?(l.value="规则已删除",h()):l.value="删除失败"}catch(s){l.value="删除失败: "+s}}async function $(){try{const e=o.value?J.value:0,s=await(await fetch("/api/alerts/silence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({minutes:e})})).json();o.value=!!(s&&s.silenced),l.value=o.value?"已静默":"已恢复推送"}catch(e){l.value="静默设置失败: "+e}}async function we(){o.value=!1,await $()}function xe(e){return!!e&&!e.degraded}const ke=Vue.computed(()=>(t&&t.analyticsRank&&t.analyticsRank.value||[]).reduce((s,a)=>Math.max(s,a.views||0),0)||1),T=()=>c().OPENAPI_ROUTE_BASE||"/api/openapi";async function D(){f.value=!0;try{const e=await c().apiFetch(T()+"/keys");I.value=e&&e.data||[]}catch(e){ElementPlus.ElMessage.error("加载 API Key 失败: "+(e.message||""))}finally{f.value=!1}}async function _e(){try{const e=await c().apiFetch(T()+"/keys",{method:"POST",body:JSON.stringify({name:g.value||"未命名",role:H.value||"read",expire_days:365})});e&&e.success?(d.value=e.api_key||"",g.value="",ElementPlus.ElMessage.success("API Key 已生成（明文仅展示一次）"),await D()):ElementPlus.ElMessage.error(e&&(e.detail||e.message)||"生成失败")}catch(e){ElementPlus.ElMessage.error("生成失败: "+(e.message||""))}}async function Ce(){if(d.value)try{await navigator.clipboard.writeText(d.value),ElementPlus.ElMessage.success("已复制")}catch{ElementPlus.ElMessage.error("复制失败，请手动复制")}}async function Ae(e){try{const s=await c().apiFetch(T()+"/keys/"+e.id,{method:"DELETE"});s&&s.success?(ElementPlus.ElMessage.success("Key 已吊销"),d.value&&e.prefix&&d.value.includes(e.prefix)&&(d.value=""),await D()):ElementPlus.ElMessage.error(s&&(s.detail||s.message)||"吊销失败")}catch(s){ElementPlus.ElMessage.error("吊销失败: "+(s.message||""))}}const ze={sxsc_tushare:"东财",tushare:"Tushare",akshare:"AkShare"};function Re(e){return ze[e]||e}const Me=computed(()=>{var e;return(((e=t.healthMetrics)==null?void 0:e.value)||[]).map(s=>({name:Re(s.name),source:s.name,success_rate:s.success_rate,avg_latency_ms:s.avg_latency_ms,calls:s.calls||0,degraded:!!s.degraded,data_age_hours:s.data_age_hours!=null?s.data_age_hours:null,stale:!!s.stale,last_fetch:s.last_fetch||s.last_success||null}))});function Pe(e){return e.degraded?"degraded":e.success_rate==null?"unknown":e.success_rate>=90?"ok":e.success_rate>=60?"warn":"bad"}function Te(e){return e==null?"":e<1?"刚刚":e<24?Math.round(e)+"小时前":Math.floor(e/24)+"天前"}const n=t.aiUsage||Vue.ref({}),S=Vue.computed(()=>{const e=n.value&&n.value.by_model||{};return Object.entries(e).map(([s,a])=>({name:s,count:a})).sort((s,a)=>a.count-s.count)}),De=Vue.computed(()=>S.value.reduce((e,s)=>Math.max(e,s.count),0)||1),Se=Vue.computed(()=>S.value.reduce((e,s)=>e+s.count,0)||1),Ve=Vue.computed(()=>V.value.reduce((e,s)=>Math.max(e,s.count),0)||0),V=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},s=[],a=new Date;for(let r=29;r>=0;r--){const E=new Date(a.getFullYear(),a.getMonth(),a.getDate()-r),X=E.getFullYear()+"-"+String(E.getMonth()+1).padStart(2,"0")+"-"+String(E.getDate()).padStart(2,"0");s.push({day:X,count:e[X]||0})}return s}),Ee=Vue.computed(()=>V.value.reduce((e,s)=>Math.max(e,s.count),0)||1),je=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},s=new Date,a=s.getFullYear()+"-"+String(s.getMonth()+1).padStart(2,"0")+"-"+String(s.getDate()).padStart(2,"0");return e[a]||0}),Ie=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},s=Object.keys(e).filter(a=>(e[a]||0)>0);return s.length?s[s.length-1]:""});function He(e){t.analyticsDays&&(t.analyticsDays.value=e),typeof t.loadAnalytics=="function"&&t.loadAnalytics()}const Le='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>',Fe='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';function Ke(e){return e?Fe:Le}return oe(),{...t,themeHues:ee,themeHueNames:j,themeMode:se,themeHue:p,onThemeModeChange:ae,setThemeHue:te,hueColor:le,hueName:ie,analyticsMaxViews:ke,aiModelRank:S,aiModelMax:De,aiDayTrend:V,aiDayMax:Ee,todayAiCalls:je,lastAiCallDay:Ie,aiTotal:Se,aiDayPeak:Ve,setAnalyticsDays:He,viewIcon:Ke,openApiKeys:I,openApiKeyName:g,openApiKeyRole:H,newOpenApiKey:d,openApiLoading:f,loadOpenApiKeys:D,generateOpenApiKey:_e,copyOpenApiKey:Ce,revokeOpenApiKey:Ae,healthRows:Me,healthClass:Pe,fmtAge:Te,staleAssetCount:me,jobQueue:F,loadJobQueue:m,cancelJob:de,jobStatusText:re,auditLogs:b,auditLoading:y,loadAuditLogs:ce,healthLoading:w,healthError:x,healthUpdatedAt:L,freshnessData:v,healHistory:k,startupReport:K,sourceHealth:O,refreshHealth:N,statusColor:ue,statusLabel:pe,sourceOk:xe,dictLoading:_,dictError:C,dictCategory:A,dictData:z,loadDataDict:U,ncTab:q,ncRules:B,ncHistory:G,ncChannels:W,ncLoading:i,ncNewCode:R,ncNewType:M,ncNewThreshold:P,ncSilence:o,ncSilenceMinutes:J,ncMsg:l,ncTypeLabel:he,onNcTab:ge,loadAlertRules:h,loadAlertHistory:Q,loadAlertChannels:Y,addAlertRule:fe,toggleAlertRule:be,removeAlertRule:ye,applySilence:$,clearSilence:we}}}})();
