(function(){const{inject:k}=Vue;window.__quantComponents=window.__quantComponents||{},window.__quantComponents.SystemPage={name:"qc-system-page",template:`
    ...
    `,setup(){const a=k("qcState");if(!a)return{};Vue.watch(()=>a.currentSubPage&&a.currentSubPage.value,e=>{e==="autoeval"&&a.loadAiVendors&&a.loadAiVendors()});const f=Vue.ref([]),c=Vue.ref(""),y=Vue.ref("read"),i=Vue.ref(""),n=Vue.ref(!1),d=()=>window.__quantModules&&window.__quantModules.core||{},r=Vue.ref([]),o=Vue.ref(!1);async function x(){o.value=!0;try{const e=await fetch("/api/audit/logs?limit=20",{headers:d().authHeaders?d().authHeaders():{}}).then(function(s){if(!s.ok)throw new Error("HTTP "+s.status);return s.json()});r.value=e&&e.logs||[]}catch(e){console.error("[system] 审计加载失败:",e),r.value=[]}finally{o.value=!1}}const w=Vue.computed(()=>(a&&a.analyticsRank&&a.analyticsRank.value||[]).reduce((s,t)=>Math.max(s,t.views||0),0)||1),v=()=>d().OPENAPI_ROUTE_BASE||"/api/openapi";async function u(){n.value=!0;try{const e=await d().apiFetch(v()+"/keys");f.value=e&&e.data||[]}catch(e){ElementPlus.ElMessage.error("加载 API Key 失败: "+(e.message||""))}finally{n.value=!1}}async function _(){try{const e=await d().apiFetch(v()+"/keys",{method:"POST",body:JSON.stringify({name:c.value||"未命名",role:y.value||"read",expire_days:365})});e&&e.success?(i.value=e.api_key||"",c.value="",ElementPlus.ElMessage.success("API Key 已生成（明文仅展示一次）"),await u()):ElementPlus.ElMessage.error(e&&(e.detail||e.message)||"生成失败")}catch(e){ElementPlus.ElMessage.error("生成失败: "+(e.message||""))}}async function C(){if(i.value)try{await navigator.clipboard.writeText(i.value),ElementPlus.ElMessage.success("已复制")}catch{ElementPlus.ElMessage.error("复制失败，请手动复制")}}async function A(e){try{const s=await d().apiFetch(v()+"/keys/"+e.id,{method:"DELETE"});s&&s.success?(ElementPlus.ElMessage.success("Key 已吊销"),i.value&&e.prefix&&i.value.includes(e.prefix)&&(i.value=""),await u()):ElementPlus.ElMessage.error(s&&(s.detail||s.message)||"吊销失败")}catch(s){ElementPlus.ElMessage.error("吊销失败: "+(s.message||""))}}const M={sxsc_tushare:"东财",tushare:"Tushare",akshare:"AkShare"};function z(e){return M[e]||e}const P=computed(()=>{var e;return(((e=a.healthMetrics)==null?void 0:e.value)||[]).map(s=>({name:z(s.name),source:s.name,success_rate:s.success_rate,avg_latency_ms:s.avg_latency_ms,calls:s.calls||0,degraded:!!s.degraded,data_age_hours:s.data_age_hours!=null?s.data_age_hours:null,stale:!!s.stale,last_fetch:s.last_fetch||s.last_success||null}))});function R(e){return e.degraded?"degraded":e.success_rate==null?"unknown":e.success_rate>=90?"ok":e.success_rate>=60?"warn":"bad"}function D(e){return e==null?"":e<1?"刚刚":e<24?Math.round(e)+"小时前":Math.floor(e/24)+"天前"}const l=a.aiUsage||Vue.ref({}),m=Vue.computed(()=>{const e=l.value&&l.value.by_model||{};return Object.entries(e).map(([s,t])=>({name:s,count:t})).sort((s,t)=>t.count-s.count)}),S=Vue.computed(()=>m.value.reduce((e,s)=>Math.max(e,s.count),0)||1),I=Vue.computed(()=>m.value.reduce((e,s)=>e+s.count,0)||1),E=Vue.computed(()=>p.value.reduce((e,s)=>Math.max(e,s.count),0)||0),p=Vue.computed(()=>{const e=l.value&&l.value.by_day||{},s=[],t=new Date;for(let g=29;g>=0;g--){const h=new Date(t.getFullYear(),t.getMonth(),t.getDate()-g),b=h.getFullYear()+"-"+String(h.getMonth()+1).padStart(2,"0")+"-"+String(h.getDate()).padStart(2,"0");s.push({day:b,count:e[b]||0})}return s}),T=Vue.computed(()=>p.value.reduce((e,s)=>Math.max(e,s.count),0)||1),V=Vue.computed(()=>{const e=l.value&&l.value.by_day||{},s=new Date,t=s.getFullYear()+"-"+String(s.getMonth()+1).padStart(2,"0")+"-"+String(s.getDate()).padStart(2,"0");return e[t]||0}),F=Vue.computed(()=>{const e=l.value&&l.value.by_day||{},s=Object.keys(e).filter(t=>(e[t]||0)>0);return s.length?s[s.length-1]:""});function K(e){a.analyticsDays&&(a.analyticsDays.value=e),typeof a.loadAnalytics=="function"&&a.loadAnalytics()}const L='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>',O='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';function U(e){return e?O:L}return{...a,analyticsMaxViews:w,aiModelRank:m,aiModelMax:S,aiDayTrend:p,aiDayMax:T,todayAiCalls:V,lastAiCallDay:F,aiTotal:I,aiDayPeak:E,setAnalyticsDays:K,viewIcon:U,openApiKeys:f,openApiKeyName:c,openApiKeyRole:y,newOpenApiKey:i,openApiLoading:n,loadOpenApiKeys:u,generateOpenApiKey:_,copyOpenApiKey:C,revokeOpenApiKey:A,healthRows:P,healthClass:R,fmtAge:D,auditLogs:r,auditLoading:o,loadAuditLogs:x}}}})();
