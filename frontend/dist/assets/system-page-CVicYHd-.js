(function(){const{inject:I}=Vue;window.__quantComponents=window.__quantComponents||{},window.__quantComponents.SystemPage={name:"qc-system-page",template:`
    ...
    `,setup(){const t=I("qcState");if(!t)return{};Vue.watch(()=>t.currentSubPage&&t.currentSubPage.value,e=>{e==="autoeval"&&t.loadAiVendors&&t.loadAiVendors(),e==="datadict"&&S()});const z=Vue.ref([]),n=Vue.ref(""),R=Vue.ref("read"),c=Vue.ref(""),o=Vue.ref(!1),l=()=>window.__quantModules&&window.__quantModules.core||{},v=Vue.ref([]),u=Vue.ref(!1);async function T(){u.value=!0;try{const e=await fetch("/api/audit/logs?limit=20",{headers:l().authHeaders?l().authHeaders():{}}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()});v.value=e&&e.logs||[]}catch(e){console.error("[system] 审计加载失败:",e),v.value=[]}finally{u.value=!1}}const p=Vue.ref(!1),m=Vue.ref(null),D=Vue.ref(null),g=Vue.ref({items:[]}),h=Vue.ref([]),M=Vue.ref(null),P=Vue.ref({data_sources:[],alerts:[]}),V=function(){return l().authHeaders?l().authHeaders():{}},r=function(e){return fetch(e,{headers:V()}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()})};async function F(){p.value=!0,m.value=null;try{const[e,a,s,d]=await Promise.all([r("/api/reliability/freshness"),r("/api/reliability/heal-history?limit=20"),r("/api/reliability/startup-report"),r("/api/reliability/source-health")]);g.value=e&&e.data||{items:[]},h.value=a&&a.data||[],M.value=s&&s.data||null,P.value=d||{data_sources:[],alerts:[]},D.value=new Date().toLocaleTimeString()}catch(e){console.warn("[health] 加载失败:",e),m.value="健康数据加载失败: "+(e.message||""),g.value={items:[]},h.value=[]}finally{p.value=!1}}const f=Vue.ref(!1),y=Vue.ref(""),b=Vue.ref(""),k=Vue.ref({fields:[]});async function S(){f.value=!0,y.value="";try{const e="/api/data-dict"+(b.value?"?category="+b.value:""),a=await r(e);k.value=a&&a.data||{fields:[]}}catch(e){console.warn("[dict] 加载失败:",e),y.value="数据字典加载失败: "+(e.message||""),k.value={fields:[]}}finally{f.value=!1}}function L(e){return{fresh:"var(--color-success)",stale:"var(--color-danger)",missing:"var(--text-tertiary)",unknown:"var(--color-warning)"}[e]||"var(--text-secondary)"}function H(e){return{fresh:"正常",stale:"过期",missing:"缺失",unknown:"未知"}[e]||e}function K(e){return!!e&&!e.degraded}const O=Vue.computed(()=>(t&&t.analyticsRank&&t.analyticsRank.value||[]).reduce((a,s)=>Math.max(a,s.views||0),0)||1),x=()=>l().OPENAPI_ROUTE_BASE||"/api/openapi";async function w(){o.value=!0;try{const e=await l().apiFetch(x()+"/keys");z.value=e&&e.data||[]}catch(e){ElementPlus.ElMessage.error("加载 API Key 失败: "+(e.message||""))}finally{o.value=!1}}async function U(){try{const e=await l().apiFetch(x()+"/keys",{method:"POST",body:JSON.stringify({name:n.value||"未命名",role:R.value||"read",expire_days:365})});e&&e.success?(c.value=e.api_key||"",n.value="",ElementPlus.ElMessage.success("API Key 已生成（明文仅展示一次）"),await w()):ElementPlus.ElMessage.error(e&&(e.detail||e.message)||"生成失败")}catch(e){ElementPlus.ElMessage.error("生成失败: "+(e.message||""))}}async function j(){if(c.value)try{await navigator.clipboard.writeText(c.value),ElementPlus.ElMessage.success("已复制")}catch{ElementPlus.ElMessage.error("复制失败，请手动复制")}}async function B(e){try{const a=await l().apiFetch(x()+"/keys/"+e.id,{method:"DELETE"});a&&a.success?(ElementPlus.ElMessage.success("Key 已吊销"),c.value&&e.prefix&&c.value.includes(e.prefix)&&(c.value=""),await w()):ElementPlus.ElMessage.error(a&&(a.detail||a.message)||"吊销失败")}catch(a){ElementPlus.ElMessage.error("吊销失败: "+(a.message||""))}}const G={sxsc_tushare:"东财",tushare:"Tushare",akshare:"AkShare"};function N(e){return G[e]||e}const W=computed(()=>{var e;return(((e=t.healthMetrics)==null?void 0:e.value)||[]).map(a=>({name:N(a.name),source:a.name,success_rate:a.success_rate,avg_latency_ms:a.avg_latency_ms,calls:a.calls||0,degraded:!!a.degraded,data_age_hours:a.data_age_hours!=null?a.data_age_hours:null,stale:!!a.stale,last_fetch:a.last_fetch||a.last_success||null}))});function q(e){return e.degraded?"degraded":e.success_rate==null?"unknown":e.success_rate>=90?"ok":e.success_rate>=60?"warn":"bad"}function Y(e){return e==null?"":e<1?"刚刚":e<24?Math.round(e)+"小时前":Math.floor(e/24)+"天前"}const i=t.aiUsage||Vue.ref({}),_=Vue.computed(()=>{const e=i.value&&i.value.by_model||{};return Object.entries(e).map(([a,s])=>({name:a,count:s})).sort((a,s)=>s.count-a.count)}),$=Vue.computed(()=>_.value.reduce((e,a)=>Math.max(e,a.count),0)||1),J=Vue.computed(()=>_.value.reduce((e,a)=>e+a.count,0)||1),Q=Vue.computed(()=>C.value.reduce((e,a)=>Math.max(e,a.count),0)||0),C=Vue.computed(()=>{const e=i.value&&i.value.by_day||{},a=[],s=new Date;for(let d=29;d>=0;d--){const A=new Date(s.getFullYear(),s.getMonth(),s.getDate()-d),E=A.getFullYear()+"-"+String(A.getMonth()+1).padStart(2,"0")+"-"+String(A.getDate()).padStart(2,"0");a.push({day:E,count:e[E]||0})}return a}),X=Vue.computed(()=>C.value.reduce((e,a)=>Math.max(e,a.count),0)||1),Z=Vue.computed(()=>{const e=i.value&&i.value.by_day||{},a=new Date,s=a.getFullYear()+"-"+String(a.getMonth()+1).padStart(2,"0")+"-"+String(a.getDate()).padStart(2,"0");return e[s]||0}),ee=Vue.computed(()=>{const e=i.value&&i.value.by_day||{},a=Object.keys(e).filter(s=>(e[s]||0)>0);return a.length?a[a.length-1]:""});function ae(e){t.analyticsDays&&(t.analyticsDays.value=e),typeof t.loadAnalytics=="function"&&t.loadAnalytics()}const se='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>',te='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';function le(e){return e?te:se}return{...t,analyticsMaxViews:O,aiModelRank:_,aiModelMax:$,aiDayTrend:C,aiDayMax:X,todayAiCalls:Z,lastAiCallDay:ee,aiTotal:J,aiDayPeak:Q,setAnalyticsDays:ae,viewIcon:le,openApiKeys:z,openApiKeyName:n,openApiKeyRole:R,newOpenApiKey:c,openApiLoading:o,loadOpenApiKeys:w,generateOpenApiKey:U,copyOpenApiKey:j,revokeOpenApiKey:B,healthRows:W,healthClass:q,fmtAge:Y,auditLogs:v,auditLoading:u,loadAuditLogs:T,healthLoading:p,healthError:m,healthUpdatedAt:D,freshnessData:g,healHistory:h,startupReport:M,sourceHealth:P,refreshHealth:F,statusColor:L,statusLabel:H,sourceOk:K,dictLoading:f,dictError:y,dictCategory:b,dictData:k,loadDataDict:S}}}})();
