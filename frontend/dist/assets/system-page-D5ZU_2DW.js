...
<!-- 数据新鲜度 -->
    ...
    `,setup(){const s=Z("qcState");if(!s)return{};Vue.watch(()=>s.currentSubPage&&s.currentSubPage.value,e=>{e==="autoeval"&&s.loadAiVendors&&s.loadAiVendors(),e==="datadict"&&U(),e==="health"&&O()});const ee=[45,220,0,140,270,320],j={45:"金色",220:"蓝色",0:"红色",140:"绿色",270:"紫色",320:"粉色"},ae=Vue.computed(()=>{const e=window.__quantModules&&window.__quantModules.preferences;return e&&e.getPreference&&e.getPreference("theme")||"system"}),p=Vue.ref(45);(function(){const e=window.__quantModules&&window.__quantModules.preferences,a=e&&e.getPreference&&e.getPreference("theme_hue");a!=null&&a!==""&&(p.value=parseInt(a,10))})();function se(e){s.changeThemeMode&&s.changeThemeMode(e)}function te(e){p.value=parseInt(e,10),s.changeThemeHue&&s.changeThemeHue(p.value)}function le(e){return"hsl("+e+", 75%, 42%)"}function ie(e){return j[e]||"自定义 "+e}function ce(e){s.setNavMode&&s.setNavMode(e)}function ne(e){s.setTabsEnabled&&s.setTabsEnabled(e)}const I=Vue.ref([]),g=Vue.ref(""),H=Vue.ref("read"),d=Vue.ref(""),f=Vue.ref(!1),c=()=>window.__quantModules&&window.__quantModules.core||{},b=Vue.ref([]),y=Vue.ref(!1);async function re(){y.value=!0;try{const e=await fetch("/api/audit/logs?limit=20",{headers:c().authHeaders?c().authHeaders():{}}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()});b.value=e&&e.logs||[]}catch(e){console.error("[system] 审计加载失败:",e),b.value=[]}finally{y.value=!1}}const w=Vue.ref(!1),x=Vue.ref(null),L=Vue.ref(null),F=Vue.ref([]),de=Vue.ref(null);function oe(e){return e==="completed"?"完成":e==="running"?"运行中":e==="pending"?"排队中":e==="cancelled"?"已取消":"失败"}async function m(){try{const a=await(await fetch("/api/jobs?limit=20")).json();a&&a.success&&(F.value=a.data&&a.data.tasks||[])}catch(e){console.warn("[system] 加载任务队列失败:",e)}}async function ve(e){try{await fetch("/api/jobs/"+e+"/cancel",{method:"POST"}),ElementPlus.ElMessage.success("已请求取消任务"),m()}catch(a){console.warn("[system] 取消任务失败:",a)}}function ue(){m(),de.value=window.setInterval(m,15e3)}const v=Vue.ref({items:[]}),k=Vue.ref([]),K=Vue.ref(null),N=Vue.ref({data_sources:[],alerts:[]}),pe=function(){return c().authHeaders?c().authHeaders():{}},u=function(e){return fetch(e,{headers:pe()}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()})};async function O(){w.value=!0,x.value=null;try{const[e,a,t,r]=await Promise.all([u("/api/reliability/freshness"),u("/api/reliability/heal-history?limit=20"),u("/api/reliability/startup-report"),u("/api/reliability/source-health")]);v.value=e&&e.data||{items:[]},k.value=a&&a.data||[],K.value=t&&t.data||null,N.value=r||{data_sources:[],alerts:[]},L.value=new Date().toLocaleTimeString()}catch(e){console.warn("[health] 加载失败:",e),x.value="健康数据加载失败: "+(e.message||""),v.value={items:[]},k.value=[]}finally{w.value=!1}}const _=Vue.ref(!1),C=Vue.ref(""),A=Vue.ref(""),z=Vue.ref({fields:[]});async function U(){_.value=!0,C.value="";try{const e="/api/data-dict"+(A.value?"?category="+A.value:""),a=await u(e);z.value=a&&a.data||{fields:[]}}catch(e){console.warn("[dict] 加载失败:",e),C.value="数据字典加载失败: "+(e.message||""),z.value={fields:[]}}finally{_.value=!1}}function me(e){return{fresh:"var(--color-success)",stale:"var(--color-danger)",missing:"var(--text-tertiary)",unknown:"var(--color-warning)"}[e]||"var(--text-secondary)"}function he(e){return{fresh:"正常",stale:"过期",missing:"缺失",unknown:"未知"}[e]||e}const ge=Vue.computed(()=>(v.value?v.value.items||[]:[]).filter(a=>a.status==="stale"||a.status==="missing").length),q=Vue.ref("rules"),B=Vue.ref([]),G=Vue.ref([]),W=Vue.ref([]),i=Vue.ref(!1),M=Vue.ref(""),R=Vue.ref("price_above"),P=Vue.ref(""),o=Vue.ref(!1),J=Vue.ref(60),l=Vue.ref("");function fe(e){return{price_above:"价格突破",price_below:"价格跌破",pct_change:"涨跌幅超",volume_surge:"量比异动",new_pool:"入池"}[e]||e}async function h(){i.value=!0;try{const e=await(await fetch("/api/alerts/rules")).json();B.value=e&&e.rules||[]}catch(e){l.value="规则加载失败: "+e}finally{i.value=!1}}async function Q(){i.value=!0;try{const e=await(await fetch("/api/alerts/history?limit=50")).json();G.value=e&&e.history||[]}catch(e){l.value="历史加载失败: "+e}finally{i.value=!1}}async function Y(){i.value=!0;try{const e=await(await fetch("/api/alerts/channels")).json(),a=await(await fetch("/api/alerts/silence")).json();W.value=e&&e.channels||[],o.value=!!(a&&a.silenced)}catch(e){l.value="通道状态加载失败: "+e}finally{i.value=!1}}function be(e){q.value=e,e==="rules"?h():e==="history"?Q():Y()}async function ye(){const e=M.value.trim();if(!e){l.value="请填写股票代码";return}i.value=!0;try{const a={stock_code:e,rule_type:R.value};if(R.value!=="new_pool"){const r=Number(P.value);if(isNaN(r)){l.value="阈值必须为数值";return}a.threshold=r}const t=await(await fetch("/api/alerts/rules",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).json();t&&t.rule?(l.value="规则已添加",M.value="",P.value="",h()):l.value=t&&t.detail||"添加失败"}catch(a){l.value="添加失败: "+a}finally{i.value=!1}}async function we(e){try{await fetch("/api/alerts/rules/"+e.id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:!e.enabled})}),e.enabled=!e.enabled}catch(a){l.value="切换失败: "+a}}async function xe(e){try{const a=await(await fetch("/api/alerts/rules/"+e.id,{method:"DELETE"})).json();a&&a.success?(l.value="规则已删除",h()):l.value="删除失败"}catch(a){l.value="删除失败: "+a}}async function $(){try{const e=o.value?J.value:0,a=await(await fetch("/api/alerts/silence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({minutes:e})})).json();o.value=!!(a&&a.silenced),l.value=o.value?"已静默":"已恢复推送"}catch(e){l.value="静默设置失败: "+e}}async function ke(){o.value=!1,await $()}function _e(e){return!!e&&!e.degraded}const Ce=Vue.computed(()=>(s&&s.analyticsRank&&s.analyticsRank.value||[]).reduce((a,t)=>Math.max(a,t.views||0),0)||1),T=()=>c().OPENAPI_ROUTE_BASE||"/api/openapi";async function D(){f.value=!0;try{const e=await c().apiFetch(T()+"/keys");I.value=e&&e.data||[]}catch(e){ElementPlus.ElMessage.error("加载 API Key 失败: "+(e.message||""))}finally{f.value=!1}}async function Ae(){try{const e=await c().apiFetch(T()+"/keys",{method:"POST",body:JSON.stringify({name:g.value||"未命名",role:H.value||"read",expire_days:365})});e&&e.success?(d.value=e.api_key||"",g.value="",ElementPlus.ElMessage.success("API Key 已生成（明文仅展示一次）"),await D()):ElementPlus.ElMessage.error(e&&(e.detail||e.message)||"生成失败")}catch(e){ElementPlus.ElMessage.error("生成失败: "+(e.message||""))}}async function ze(){if(d.value)try{await navigator.clipboard.writeText(d.value),ElementPlus.ElMessage.success("已复制")}catch{ElementPlus.ElMessage.error("复制失败，请手动复制")}}async function Me(e){try{const a=await c().apiFetch(T()+"/keys/"+e.id,{method:"DELETE"});a&&a.success?(ElementPlus.ElMessage.success("Key 已吊销"),d.value&&e.prefix&&d.value.includes(e.prefix)&&(d.value=""),await D()):ElementPlus.ElMessage.error(a&&(a.detail||a.message)||"吊销失败")}catch(a){ElementPlus.ElMessage.error("吊销失败: "+(a.message||""))}}const Re={sxsc_tushare:"东财",tushare:"Tushare",akshare:"AkShare"};function Pe(e){return Re[e]||e}const Te=computed(()=>{var e;return(((e=s.healthMetrics)==null?void 0:e.value)||[]).map(a=>({name:Pe(a.name),source:a.name,success_rate:a.success_rate,avg_latency_ms:a.avg_latency_ms,calls:a.calls||0,degraded:!!a.degraded,data_age_hours:a.data_age_hours!=null?a.data_age_hours:null,stale:!!a.stale,last_fetch:a.last_fetch||a.last_success||null}))});function De(e){return e.degraded?"degraded":e.success_rate==null?"unknown":e.success_rate>=90?"ok":e.success_rate>=60?"warn":"bad"}function Se(e){return e==null?"":e<1?"刚刚":e<24?Math.round(e)+"小时前":Math.floor(e/24)+"天前"}const n=s.aiUsage||Vue.ref({}),S=Vue.computed(()=>{const e=n.value&&n.value.by_model||{};return Object.entries(e).map(([a,t])=>({name:a,count:t})).sort((a,t)=>t.count-a.count)}),Ee=Vue.computed(()=>S.value.reduce((e,a)=>Math.max(e,a.count),0)||1),Ve=Vue.computed(()=>S.value.reduce((e,a)=>e+a.count,0)||1),je=Vue.computed(()=>E.value.reduce((e,a)=>Math.max(e,a.count),0)||0),E=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=[],t=new Date;for(let r=29;r>=0;r--){const V=new Date(t.getFullYear(),t.getMonth(),t.getDate()-r),X=V.getFullYear()+"-"+String(V.getMonth()+1).padStart(2,"0")+"-"+String(V.getDate()).padStart(2,"0");a.push({day:X,count:e[X]||0})}return a}),Ie=Vue.computed(()=>E.value.reduce((e,a)=>Math.max(e,a.count),0)||1),He=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=new Date,t=a.getFullYear()+"-"+String(a.getMonth()+1).padStart(2,"0")+"-"+String(a.getDate()).padStart(2,"0");return e[t]||0}),Le=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=Object.keys(e).filter(t=>(e[t]||0)>0);return a.length?a[a.length-1]:""});function Fe(e){s.analyticsDays&&(s.analyticsDays.value=e),typeof s.loadAnalytics=="function"&&s.loadAnalytics()}const Ke='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>',Ne='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';function Oe(e){return e?Ne:Ke}return ue(),{...s,themeHues:ee,themeHueNames:j,themeMode:ae,themeHue:p,onThemeModeChange:se,setThemeHue:te,hueColor:le,hueName:ie,onNavModeChange:ce,onTabsEnabledChange:ne,analyticsMaxViews:Ce,aiModelRank:S,aiModelMax:Ee,aiDayTrend:E,aiDayMax:Ie,todayAiCalls:He,lastAiCallDay:Le,aiTotal:Ve,aiDayPeak:je,setAnalyticsDays:Fe,viewIcon:Oe,openApiKeys:I,openApiKeyName:g,openApiKeyRole:H,newOpenApiKey:d,openApiLoading:f,loadOpenApiKeys:D,generateOpenApiKey:Ae,copyOpenApiKey:ze,revokeOpenApiKey:Me,healthRows:Te,healthClass:De,fmtAge:Se,staleAssetCount:ge,jobQueue:F,loadJobQueue:m,cancelJob:ve,jobStatusText:oe,auditLogs:b,auditLoading:y,loadAuditLogs:re,healthLoading:w,healthError:x,healthUpdatedAt:L,freshnessData:v,healHistory:k,startupReport:K,sourceHealth:N,refreshHealth:O,statusColor:me,statusLabel:he,sourceOk:_e,dictLoading:_,dictError:C,dictCategory:A,dictData:z,loadDataDict:U,ncTab:q,ncRules:B,ncHistory:G,ncChannels:W,ncLoading:i,ncNewCode:M,ncNewType:R,ncNewThreshold:P,ncSilence:o,ncSilenceMinutes:J,ncMsg:l,ncTypeLabel:fe,onNcTab:be,loadAlertRules:h,loadAlertHistory:Q,loadAlertChannels:Y,addAlertRule:ye,toggleAlertRule:we,removeAlertRule:xe,applySilence:$,clearSilence:ke}}}})();
