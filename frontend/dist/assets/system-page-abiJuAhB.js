(function(){const{inject:J}=Vue;window.__quantComponents=window.__quantComponents||{},window.__quantComponents.SystemPage={name:"qc-system-page",template:`
    ...
    `,setup(){const l=J("qcState");if(!l)return{};Vue.watch(()=>l.currentSubPage&&l.currentSubPage.value,e=>{e==="autoeval"&&l.loadAiVendors&&l.loadAiVendors(),e==="datadict"&&H()});const E=Vue.ref([]),p=Vue.ref(""),V=Vue.ref("read"),o=Vue.ref(""),m=Vue.ref(!1),c=()=>window.__quantModules&&window.__quantModules.core||{},g=Vue.ref([]),h=Vue.ref(!1);async function Y(){h.value=!0;try{const e=await fetch("/api/audit/logs?limit=20",{headers:c().authHeaders?c().authHeaders():{}}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()});g.value=e&&e.logs||[]}catch(e){console.error("[system] 审计加载失败:",e),g.value=[]}finally{h.value=!1}}const f=Vue.ref(!1),b=Vue.ref(null),I=Vue.ref(null),y=Vue.ref({items:[]}),k=Vue.ref([]),L=Vue.ref(null),F=Vue.ref({data_sources:[],alerts:[]}),$=function(){return c().authHeaders?c().authHeaders():{}},v=function(e){return fetch(e,{headers:$()}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()})};async function Q(){f.value=!0,b.value=null;try{const[e,a,s,r]=await Promise.all([v("/api/reliability/freshness"),v("/api/reliability/heal-history?limit=20"),v("/api/reliability/startup-report"),v("/api/reliability/source-health")]);y.value=e&&e.data||{items:[]},k.value=a&&a.data||[],L.value=s&&s.data||null,F.value=r||{data_sources:[],alerts:[]},I.value=new Date().toLocaleTimeString()}catch(e){console.warn("[health] 加载失败:",e),b.value="健康数据加载失败: "+(e.message||""),y.value={items:[]},k.value=[]}finally{f.value=!1}}const x=Vue.ref(!1),w=Vue.ref(""),_=Vue.ref(""),C=Vue.ref({fields:[]});async function H(){x.value=!0,w.value="";try{const e="/api/data-dict"+(_.value?"?category="+_.value:""),a=await v(e);C.value=a&&a.data||{fields:[]}}catch(e){console.warn("[dict] 加载失败:",e),w.value="数据字典加载失败: "+(e.message||""),C.value={fields:[]}}finally{x.value=!1}}function X(e){return{fresh:"var(--color-success)",stale:"var(--color-danger)",missing:"var(--text-tertiary)",unknown:"var(--color-warning)"}[e]||"var(--text-secondary)"}function Z(e){return{fresh:"正常",stale:"过期",missing:"缺失",unknown:"未知"}[e]||e}const K=Vue.ref("rules"),O=Vue.ref([]),j=Vue.ref([]),N=Vue.ref([]),i=Vue.ref(!1),A=Vue.ref(""),z=Vue.ref("price_above"),R=Vue.ref(""),d=Vue.ref(!1),U=Vue.ref(60),t=Vue.ref("");function ee(e){return{price_above:"价格突破",price_below:"价格跌破",pct_change:"涨跌幅超",volume_surge:"量比异动",new_pool:"入池"}[e]||e}async function u(){i.value=!0;try{const e=await(await fetch("/api/alerts/rules")).json();O.value=e&&e.rules||[]}catch(e){t.value="规则加载失败: "+e}finally{i.value=!1}}async function B(){i.value=!0;try{const e=await(await fetch("/api/alerts/history?limit=50")).json();j.value=e&&e.history||[]}catch(e){t.value="历史加载失败: "+e}finally{i.value=!1}}async function G(){i.value=!0;try{const e=await(await fetch("/api/alerts/channels")).json(),a=await(await fetch("/api/alerts/silence")).json();N.value=e&&e.channels||[],d.value=!!(a&&a.silenced)}catch(e){t.value="通道状态加载失败: "+e}finally{i.value=!1}}function ae(e){K.value=e,e==="rules"?u():e==="history"?B():G()}async function se(){const e=A.value.trim();if(!e){t.value="请填写股票代码";return}i.value=!0;try{const a={stock_code:e,rule_type:z.value};if(z.value!=="new_pool"){const r=Number(R.value);if(isNaN(r)){t.value="阈值必须为数值";return}a.threshold=r}const s=await(await fetch("/api/alerts/rules",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).json();s&&s.rule?(t.value="规则已添加",A.value="",R.value="",u()):t.value=s&&s.detail||"添加失败"}catch(a){t.value="添加失败: "+a}finally{i.value=!1}}async function le(e){try{await fetch("/api/alerts/rules/"+e.id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:!e.enabled})}),e.enabled=!e.enabled}catch(a){t.value="切换失败: "+a}}async function te(e){try{const a=await(await fetch("/api/alerts/rules/"+e.id,{method:"DELETE"})).json();a&&a.success?(t.value="规则已删除",u()):t.value="删除失败"}catch(a){t.value="删除失败: "+a}}async function W(){try{const e=d.value?U.value:0,a=await(await fetch("/api/alerts/silence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({minutes:e})})).json();d.value=!!(a&&a.silenced),t.value=d.value?"已静默":"已恢复推送"}catch(e){t.value="静默设置失败: "+e}}async function ie(){d.value=!1,await W()}function ce(e){return!!e&&!e.degraded}const ne=Vue.computed(()=>(l&&l.analyticsRank&&l.analyticsRank.value||[]).reduce((a,s)=>Math.max(a,s.views||0),0)||1),S=()=>c().OPENAPI_ROUTE_BASE||"/api/openapi";async function M(){m.value=!0;try{const e=await c().apiFetch(S()+"/keys");E.value=e&&e.data||[]}catch(e){ElementPlus.ElMessage.error("加载 API Key 失败: "+(e.message||""))}finally{m.value=!1}}async function re(){try{const e=await c().apiFetch(S()+"/keys",{method:"POST",body:JSON.stringify({name:p.value||"未命名",role:V.value||"read",expire_days:365})});e&&e.success?(o.value=e.api_key||"",p.value="",ElementPlus.ElMessage.success("API Key 已生成（明文仅展示一次）"),await M()):ElementPlus.ElMessage.error(e&&(e.detail||e.message)||"生成失败")}catch(e){ElementPlus.ElMessage.error("生成失败: "+(e.message||""))}}async function oe(){if(o.value)try{await navigator.clipboard.writeText(o.value),ElementPlus.ElMessage.success("已复制")}catch{ElementPlus.ElMessage.error("复制失败，请手动复制")}}async function de(e){try{const a=await c().apiFetch(S()+"/keys/"+e.id,{method:"DELETE"});a&&a.success?(ElementPlus.ElMessage.success("Key 已吊销"),o.value&&e.prefix&&o.value.includes(e.prefix)&&(o.value=""),await M()):ElementPlus.ElMessage.error(a&&(a.detail||a.message)||"吊销失败")}catch(a){ElementPlus.ElMessage.error("吊销失败: "+(a.message||""))}}const ve={sxsc_tushare:"东财",tushare:"Tushare",akshare:"AkShare"};function ue(e){return ve[e]||e}const pe=computed(()=>{var e;return(((e=l.healthMetrics)==null?void 0:e.value)||[]).map(a=>({name:ue(a.name),source:a.name,success_rate:a.success_rate,avg_latency_ms:a.avg_latency_ms,calls:a.calls||0,degraded:!!a.degraded,data_age_hours:a.data_age_hours!=null?a.data_age_hours:null,stale:!!a.stale,last_fetch:a.last_fetch||a.last_success||null}))});function me(e){return e.degraded?"degraded":e.success_rate==null?"unknown":e.success_rate>=90?"ok":e.success_rate>=60?"warn":"bad"}function ge(e){return e==null?"":e<1?"刚刚":e<24?Math.round(e)+"小时前":Math.floor(e/24)+"天前"}const n=l.aiUsage||Vue.ref({}),D=Vue.computed(()=>{const e=n.value&&n.value.by_model||{};return Object.entries(e).map(([a,s])=>({name:a,count:s})).sort((a,s)=>s.count-a.count)}),he=Vue.computed(()=>D.value.reduce((e,a)=>Math.max(e,a.count),0)||1),fe=Vue.computed(()=>D.value.reduce((e,a)=>e+a.count,0)||1),be=Vue.computed(()=>P.value.reduce((e,a)=>Math.max(e,a.count),0)||0),P=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=[],s=new Date;for(let r=29;r>=0;r--){const T=new Date(s.getFullYear(),s.getMonth(),s.getDate()-r),q=T.getFullYear()+"-"+String(T.getMonth()+1).padStart(2,"0")+"-"+String(T.getDate()).padStart(2,"0");a.push({day:q,count:e[q]||0})}return a}),ye=Vue.computed(()=>P.value.reduce((e,a)=>Math.max(e,a.count),0)||1),ke=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=new Date,s=a.getFullYear()+"-"+String(a.getMonth()+1).padStart(2,"0")+"-"+String(a.getDate()).padStart(2,"0");return e[s]||0}),xe=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=Object.keys(e).filter(s=>(e[s]||0)>0);return a.length?a[a.length-1]:""});function we(e){l.analyticsDays&&(l.analyticsDays.value=e),typeof l.loadAnalytics=="function"&&l.loadAnalytics()}const _e='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>',Ce='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';function Ae(e){return e?Ce:_e}return{...l,analyticsMaxViews:ne,aiModelRank:D,aiModelMax:he,aiDayTrend:P,aiDayMax:ye,todayAiCalls:ke,lastAiCallDay:xe,aiTotal:fe,aiDayPeak:be,setAnalyticsDays:we,viewIcon:Ae,openApiKeys:E,openApiKeyName:p,openApiKeyRole:V,newOpenApiKey:o,openApiLoading:m,loadOpenApiKeys:M,generateOpenApiKey:re,copyOpenApiKey:oe,revokeOpenApiKey:de,healthRows:pe,healthClass:me,fmtAge:ge,auditLogs:g,auditLoading:h,loadAuditLogs:Y,healthLoading:f,healthError:b,healthUpdatedAt:I,freshnessData:y,healHistory:k,startupReport:L,sourceHealth:F,refreshHealth:Q,statusColor:X,statusLabel:Z,sourceOk:ce,dictLoading:x,dictError:w,dictCategory:_,dictData:C,loadDataDict:H,ncTab:K,ncRules:O,ncHistory:j,ncChannels:N,ncLoading:i,ncNewCode:A,ncNewType:z,ncNewThreshold:R,ncSilence:d,ncSilenceMinutes:U,ncMsg:t,ncTypeLabel:ee,onNcTab:ae,loadAlertRules:u,loadAlertHistory:B,loadAlertChannels:G,addAlertRule:se,toggleAlertRule:le,removeAlertRule:te,applySilence:W,clearSilence:ie}}}})();
