(function(){const{inject:X}=Vue;window.__quantComponents=window.__quantComponents||{},window.__quantComponents.SystemPage={name:"qc-system-page",template:`
...
<!-- 数据新鲜度 -->
    ...
    `,setup(){const s=X("qcState");if(!s)return{};function Z(e){s.currentSubPage.value=e}function ee(){p(),M(),D()}Vue.watch(()=>s.currentSubPage&&s.currentSubPage.value,e=>{e==="autoeval"&&s.loadAiVendors&&s.loadAiVendors(),e==="datadict"&&U(),e==="health"&&O(),e==="notification"&&ee()});const ae=s.themeHues||[45,220,0,140,270,320],H=s.themeHueNames||{},se=s.themeMode||Vue.computed(()=>"light"),te=s.themeHue||Vue.ref(45);function le(e){s.changeThemeMode&&s.changeThemeMode(e)}function ie(e){s.changeThemeHue&&s.changeThemeHue(parseInt(e,10))}function ce(e){return s.hueColor?s.hueColor(e):"hsl("+e+", 75%, 42%)"}function ne(e){return s.hueName?s.hueName(e):H[e]||"自定义 "+e}function oe(e){s.setNavMode&&s.setNavMode(e)}const j=Vue.ref([]),g=Vue.ref(""),I=Vue.ref("read"),r=Vue.ref(""),h=Vue.ref(!1),c=()=>window.__quantModules&&window.__quantModules.core||{},f=Vue.ref([]),b=Vue.ref(!1);async function re(){b.value=!0;try{const e=await fetch("/api/audit/logs?limit=20",{headers:c().authHeaders?c().authHeaders():{}}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()});f.value=e&&e.logs||[]}catch(e){console.error("[system] 审计加载失败:",e),f.value=[]}finally{b.value=!1}}const y=Vue.ref(!1),x=Vue.ref(null),L=Vue.ref(null),F=Vue.ref([]),de=Vue.ref(null);function ue(e){return e==="completed"?"完成":e==="running"?"运行中":e==="pending"?"排队中":e==="cancelled"?"已取消":"失败"}async function m(){try{const a=await(await fetch("/api/jobs?limit=20")).json();a&&a.success&&(F.value=a.data&&a.data.tasks||[])}catch(e){console.warn("[system] 加载任务队列失败:",e)}}async function ve(e){try{await fetch("/api/jobs/"+e+"/cancel",{method:"POST"}),ElementPlus.ElMessage.success("已请求取消任务"),m()}catch(a){console.warn("[system] 取消任务失败:",a)}}function pe(){m(),de.value=window.setInterval(m,15e3)}const u=Vue.ref({items:[]}),k=Vue.ref([]),N=Vue.ref(null),K=Vue.ref({data_sources:[],alerts:[]}),me=function(){return c().authHeaders?c().authHeaders():{}},v=function(e){return fetch(e,{headers:me()}).then(function(a){if(!a.ok)throw new Error("HTTP "+a.status);return a.json()})};async function O(){y.value=!0,x.value=null;try{const[e,a,t,o]=await Promise.all([v("/api/reliability/freshness"),v("/api/reliability/heal-history?limit=20"),v("/api/reliability/startup-report"),v("/api/reliability/source-health")]);u.value=e&&e.data||{items:[]},k.value=a&&a.data||[],N.value=t&&t.data||null,K.value=o||{data_sources:[],alerts:[]},L.value=new Date().toLocaleTimeString()}catch(e){console.warn("[health] 加载失败:",e),x.value="健康数据加载失败: "+(e.message||""),u.value={items:[]},k.value=[]}finally{y.value=!1}}const w=Vue.ref(!1),_=Vue.ref(""),z=Vue.ref(""),C=Vue.ref({fields:[]});async function U(){w.value=!0,_.value="";try{const e="/api/data-dict"+(z.value?"?category="+z.value:""),a=await v(e);C.value=a&&a.data||{fields:[]}}catch(e){console.warn("[dict] 加载失败:",e),_.value="数据字典加载失败: "+(e.message||""),C.value={fields:[]}}finally{w.value=!1}}function ge(e){return{fresh:"var(--color-success)",stale:"var(--color-danger)",missing:"var(--text-tertiary)",unknown:"var(--color-warning)"}[e]||"var(--text-secondary)"}function he(e){return{fresh:"正常",stale:"过期",missing:"缺失",unknown:"未知"}[e]||e}const fe=Vue.computed(()=>(u.value?u.value.items||[]:[]).filter(a=>a.status==="stale"||a.status==="missing").length),G=Vue.ref("rules"),W=Vue.ref([]),B=Vue.ref([]),J=Vue.ref([]),i=Vue.ref(!1),q=Vue.ref(""),A=Vue.ref("price_above"),P=Vue.ref(""),d=Vue.ref(!1),Q=Vue.ref(60),l=Vue.ref("");function be(e){return{price_above:"价格突破",price_below:"价格跌破",pct_change:"涨跌幅超",volume_surge:"量比异动",new_pool:"入池"}[e]||e}async function p(){i.value=!0;try{const e=await(await fetch("/api/alerts/rules")).json();W.value=e&&e.rules||[]}catch(e){l.value="规则加载失败: "+e}finally{i.value=!1}}async function M(){i.value=!0;try{const e=await(await fetch("/api/alerts/history?limit=50")).json();B.value=e&&e.history||[]}catch(e){l.value="历史加载失败: "+e}finally{i.value=!1}}async function D(){i.value=!0;try{const e=await(await fetch("/api/alerts/channels")).json(),a=await(await fetch("/api/alerts/silence")).json();J.value=e&&e.channels||[],d.value=!!(a&&a.silenced)}catch(e){l.value="通道状态加载失败: "+e}finally{i.value=!1}}function ye(e){G.value=e,e==="rules"?p():e==="history"?M():D()}async function xe(){const e=q.value.trim();if(!e){l.value="请填写股票代码";return}i.value=!0;try{const a={stock_code:e,rule_type:A.value};if(A.value!=="new_pool"){const o=Number(P.value);if(isNaN(o)){l.value="阈值必须为数值";return}a.threshold=o}const t=await(await fetch("/api/alerts/rules",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(a)})).json();t&&t.rule?(l.value="规则已添加",q.value="",P.value="",p()):l.value=t&&t.detail||"添加失败"}catch(a){l.value="添加失败: "+a}finally{i.value=!1}}async function ke(e){try{await fetch("/api/alerts/rules/"+e.id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({enabled:!e.enabled})}),e.enabled=!e.enabled}catch(a){l.value="切换失败: "+a}}async function we(e){try{const a=await(await fetch("/api/alerts/rules/"+e.id,{method:"DELETE"})).json();a&&a.success?(l.value="规则已删除",p()):l.value="删除失败"}catch(a){l.value="删除失败: "+a}}async function Y(){try{const e=d.value?Q.value:0,a=await(await fetch("/api/alerts/silence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({minutes:e})})).json();d.value=!!(a&&a.silenced),l.value=d.value?"已静默":"已恢复推送"}catch(e){l.value="静默设置失败: "+e}}async function _e(){d.value=!1,await Y()}function ze(e){return!!e&&!e.degraded}const Ce=Vue.computed(()=>(s&&s.analyticsRank&&s.analyticsRank.value||[]).reduce((a,t)=>Math.max(a,t.views||0),0)||1),R=()=>c().OPENAPI_ROUTE_BASE||"/api/openapi";async function S(){h.value=!0;try{const e=await c().apiFetch(R()+"/keys");j.value=e&&e.data||[]}catch(e){ElementPlus.ElMessage.error("加载 API Key 失败: "+(e.message||""))}finally{h.value=!1}}async function qe(){try{const e=await c().apiFetch(R()+"/keys",{method:"POST",body:JSON.stringify({name:g.value||"未命名",role:I.value||"read",expire_days:365})});e&&e.success?(r.value=e.api_key||"",g.value="",ElementPlus.ElMessage.success("API Key 已生成（明文仅展示一次）"),await S()):ElementPlus.ElMessage.error(e&&(e.detail||e.message)||"生成失败")}catch(e){ElementPlus.ElMessage.error("生成失败: "+(e.message||""))}}async function Ae(){if(r.value)try{await navigator.clipboard.writeText(r.value),ElementPlus.ElMessage.success("已复制")}catch{ElementPlus.ElMessage.error("复制失败，请手动复制")}}async function Pe(e){try{const a=await c().apiFetch(R()+"/keys/"+e.id,{method:"DELETE"});a&&a.success?(ElementPlus.ElMessage.success("Key 已吊销"),r.value&&e.prefix&&r.value.includes(e.prefix)&&(r.value=""),await S()):ElementPlus.ElMessage.error(a&&(a.detail||a.message)||"吊销失败")}catch(a){ElementPlus.ElMessage.error("吊销失败: "+(a.message||""))}}const Me={sxsc_tushare:"东财",tushare:"Tushare",akshare:"AkShare"};function De(e){return Me[e]||e}const Re=computed(()=>{var e;return(((e=s.healthMetrics)==null?void 0:e.value)||[]).map(a=>({name:De(a.name),source:a.name,success_rate:a.success_rate,avg_latency_ms:a.avg_latency_ms,calls:a.calls||0,degraded:!!a.degraded,data_age_hours:a.data_age_hours!=null?a.data_age_hours:null,stale:!!a.stale,last_fetch:a.last_fetch||a.last_success||null}))});function Se(e){return e.degraded?"degraded":e.success_rate==null?"unknown":e.success_rate>=90?"ok":e.success_rate>=60?"warn":"bad"}function Te(e){return e==null?"":e<1?"刚刚":e<24?Math.round(e)+"小时前":Math.floor(e/24)+"天前"}const n=s.aiUsage||Vue.ref({}),T=Vue.computed(()=>{const e=n.value&&n.value.by_model||{};return Object.entries(e).map(([a,t])=>({name:a,count:t})).sort((a,t)=>t.count-a.count)}),Ve=Vue.computed(()=>T.value.reduce((e,a)=>Math.max(e,a.count),0)||1),Ee=Vue.computed(()=>T.value.reduce((e,a)=>e+a.count,0)||1),He=Vue.computed(()=>V.value.reduce((e,a)=>Math.max(e,a.count),0)||0),V=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=[],t=new Date;for(let o=29;o>=0;o--){const E=new Date(t.getFullYear(),t.getMonth(),t.getDate()-o),$=E.getFullYear()+"-"+String(E.getMonth()+1).padStart(2,"0")+"-"+String(E.getDate()).padStart(2,"0");a.push({day:$,count:e[$]||0})}return a}),je=Vue.computed(()=>V.value.reduce((e,a)=>Math.max(e,a.count),0)||1),Ie=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=new Date,t=a.getFullYear()+"-"+String(a.getMonth()+1).padStart(2,"0")+"-"+String(a.getDate()).padStart(2,"0");return e[t]||0}),Le=Vue.computed(()=>{const e=n.value&&n.value.by_day||{},a=Object.keys(e).filter(t=>(e[t]||0)>0);return a.length?a[a.length-1]:""});function Fe(e){s.analyticsDays&&(s.analyticsDays.value=e),typeof s.loadAnalytics=="function"&&s.loadAnalytics()}const Ne='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>',Ke='<svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><line x1="1" y1="1" x2="23" y2="23"/></svg>';function Oe(e){return e?Ke:Ne}return pe(),{...s,themeHues:ae,themeHueNames:H,themeMode:se,themeHue:te,onThemeModeChange:le,setThemeHue:ie,hueColor:ce,hueName:ne,onNavModeChange:oe,analyticsMaxViews:Ce,aiModelRank:T,aiModelMax:Ve,aiDayTrend:V,aiDayMax:je,todayAiCalls:Ie,lastAiCallDay:Le,aiTotal:Ee,aiDayPeak:He,setAnalyticsDays:Fe,viewIcon:Oe,openApiKeys:j,openApiKeyName:g,openApiKeyRole:I,newOpenApiKey:r,openApiLoading:h,loadOpenApiKeys:S,generateOpenApiKey:qe,copyOpenApiKey:Ae,revokeOpenApiKey:Pe,healthRows:Re,healthClass:Se,fmtAge:Te,staleAssetCount:fe,jobQueue:F,loadJobQueue:m,cancelJob:ve,jobStatusText:ue,auditLogs:f,auditLoading:b,loadAuditLogs:re,healthLoading:y,healthError:x,healthUpdatedAt:L,freshnessData:u,healHistory:k,startupReport:N,sourceHealth:K,refreshHealth:O,statusColor:ge,statusLabel:he,sourceOk:ze,dictLoading:w,dictError:_,dictCategory:z,dictData:C,loadDataDict:U,ncTab:G,ncRules:W,ncHistory:B,ncChannels:J,ncLoading:i,ncNewCode:q,ncNewType:A,ncNewThreshold:P,ncSilence:d,ncSilenceMinutes:Q,ncMsg:l,ncTypeLabel:be,onNcTab:ye,loadAlertRules:p,loadAlertHistory:M,loadAlertChannels:D,addAlertRule:xe,toggleAlertRule:ke,removeAlertRule:we,applySilence:Y,clearSilence:_e,goSystemSub:Z}}}})();
