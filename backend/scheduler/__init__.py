...
def run_strategy_once(progress_cb=None):
    ...
    V5.3.14 (T-SP.P2.5): 非交易日(周末/法定节假日)跳过 — 防止按 today(周六/日)
    建 data/holdings/{date} 残留目录与内容日期错位(实测 9/5、9/6 周末目录).
    """
    import strategy_governance as gov
    try:
        import stock_calendar as _sc
        if not _sc.is_trade_date_str(datetime.now().strftime('%Y-%m-%d')):
            logger.info('非交易日, 跳过策略定期运行')
            return True, [], []
    except Exception as _e:
        logger.warning('交易日判断失败, 照常运行: %s', _e)
    state = gov.get_state()
    executed = []
    errors = []
