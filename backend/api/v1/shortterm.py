...
async def get_latest_session(user: Optional[dict] = Depends(get_current_user)):
    return {'success': True, 'date': latest_session()}


@router.get("/pools")
async def get_pools(date: str = None, user: Optional[dict] = Depends(get_current_user)):
...
@router.get("/dates")
async def get_dates(user: Optional[dict] = Depends(get_current_user)):
...
@router.get("/emotion")
async def get_emotion(date: str = None, user: Optional[dict] = Depends(get_current_user)):
    d = date or latest_session()
...
async def get_market_facts(date: str = None, user: Optional[dict] = Depends(get_current_user)):
    d = date or latest_session()
...
@router.get("/weekly")
async def get_weekly(end: str = None, user: Optional[dict] = Depends(get_current_user)):
...
@router.get("/review")
async def get_review(date: str = None, user: Optional[dict] = Depends(get_current_user)):
    """读取已落盘复盘; 无 → None"""
    d = date or latest_session()
...
async def get_review_dates(user: Optional[dict] = Depends(get_current_user)):
    """复盘历史检索: 已落盘复盘日期列表"""
...
async def get_reflection(date: str = None, user: Optional[dict] = Depends(get_current_user)):
...
@router.get("/intraday")
async def get_intraday(date: str = None, user: Optional[dict] = Depends(get_current_user)):
    """某日已采集的盘中快照列表(按时点排序)"""
    from datetime import datetime
    d = date or datetime.now().strftime('%Y-%m-%d')
    snapshots = []
