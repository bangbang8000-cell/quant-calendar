...
async def market_scan(date: Optional[str] = None, pool: str = "all",
                      # V5.3.15 (T-MR.2): 可选鉴权 — 游客可看 strategies/all 池, 自选需登录
...
async def market_events(scope: str = "watchlist",
                        # V5.3.15 (T-MR.2): 可选鉴权 — 游客可看 events, 自选隔离需登录
                        user: Optional[dict] = Depends(get_current_user)):
    ...
    # V5.3.15 (T-MR.2): 匿名(未登录)时无自选隔离 — 返回空事件, 不抛错
    if user is None:
        return {"success": True, "data": {"events": [], "note": "未登录, 仅展示已登录用户的持仓事件", "scope": scope, "tracked_count": 0}}
