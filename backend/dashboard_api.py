...
class DashboardAnalyzer:
    ...
    @property
    def real_latest_date(self):
        """最近真实持仓日（排除前向填充的今天/未来日期）。

        get_available_dates() 会前向填充到今天(策略未运行时 latest=今天, 当日池=继承上一
        持仓日), 导致今日一屏"入池/出池"与昨日对比恒为 0 (FIX: 2026-09-08).
        date_list 是 holdings_data 里真实有数据的日期, 取其末位即最近真实持仓日。
        """
        _dates = getattr(parser, 'date_list', None) or []
        if _dates:
            return _dates[-1]
        _avail = parser.get_available_dates()
    ...
    def get_overview(self):
        ...
        latest_date = self.real_latest_date
