# -*- coding: utf-8 -*-
"""V6.9.4 (PRD-v5.14.4，原 PRD-v6.9.4): 存量问题整改契约测试 — 共识条移除 / EP CSS 对齐 / 导航面板精简 / 主题模式响应式 / 深色输入框 / 策略研究可诊断 / 宽度类收敛。

覆盖 TEST-PLAN-v5.14.4 的 L1/L2 可静态断言部分 (视觉/交互项以浏览器实测为准)。
"""
import os
import re
...
def test_version_bumped_694():
    """T32: APP_VERSION 提升 (6.9.4 → 5.5.0, 编号并入 5.X)"""
    assert 'APP_VERSION = "5.5.0"' in _read_b("main_new.py"), "APP_VERSION 应为 5.5.0"
