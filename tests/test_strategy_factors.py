...
def test_api_factor_detail_endpoint(authed_client, monkeypatch):
    """POST /api/strategies/factors/detail → 因子详情面板 (定义/覆盖度/IC衰减/换手/多重检验/近2年)"""
    # 确定性: 强制 FakePortal 降级(不依赖 CI 网络/真实数据), 与 _resolve_portal 降级语义一致
    from strategy_sdk.testsupport import FakePortal
    from api.v1 import strategy_research
    import pandas as pd
    def _fake(universe=None, seed=2026):
        dates = pd.date_range('2026-04-01', '2026-07-31', 40).strftime('%Y-%m-%d').tolist()
        symbols = universe or [f'{600000 + i:06d}.SH' for i in range(24)]
        return FakePortal(dates=dates, symbols=symbols, seed=seed), False
    monkeypatch.setattr(strategy_research, '_resolve_portal', _fake)
    client = authed_client
