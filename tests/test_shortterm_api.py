...
def test_pools_anonymous_allowed():
    """V5.3.15 (T-MR.3): 涨停复盘为市场级只读数据, 匿名可访问(不再 401)."""
    from unittest.mock import patch
    c = _make_client(override_auth=False)
    with patch.object(fetchers, 'fetch_zt_pool', _fake_pool), \
         patch.object(fetchers, 'fetch_zb_pool', _fake_pool), \
         patch.object(fetchers, 'fetch_dt_pool', _fake_pool):
        r = c.get('/api/shortterm/pools?date=2026-09-02')
