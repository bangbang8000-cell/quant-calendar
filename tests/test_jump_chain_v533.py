...
def test_stock_detail_no_goto_calendar():
    """股票详情弹窗已移除「跳转日历」 (V5.3.13 用户建议移除; 防回退)"""
    src = _read("js/components/dialogs/stock-detail.js")
    assert "gotoCalendar" not in src, "「跳转日历」按钮已按用户建议移除"
    assert "navigateTo('calendar'" not in src
