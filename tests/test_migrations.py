...
def test_rollback_partial(conn):
    ...
    assert rolled == list(range(latest_version(), 2, -1))  # 动态: 从最新回滚到 2
    assert get_current_version(conn) == 2
...
def test_rollback_all(conn):
    ...
    assert rolled == list(range(latest_version(), 0, -1))  # 动态: 全量回滚
...
def test_rollback_roundtrip_reupgrade(conn):
    ...
    applied = upgrade(conn)
    assert applied == list(range(1, latest_version() + 1))  # 动态: 全量升级
...
def test_validate_partial_upgrade_is_ok_prefix(conn):
    ...
    v = validate(conn)
    assert v["ok"] is True
    assert v["missing"] == list(range(2, latest_version() + 1))  # 动态: 缺全部后续迁移
    assert v["current"] == 1
...
def test_failing_upgrade_raises_and_rolls_back(conn, monkeypatch):
    ...
    real_latest = latest_version()  # monkeypatch 前快照真实最新版本
    ...
    assert get_current_version(conn) == real_latest  # 动态: 原最新版本 (快照)
