...
def test_qcstate_key_count_stable():
    ...
    keys = _extract_qcstate_keys(_read("js/app-logic.js"))
    assert len(set(keys)) == 494, f"qcState 唯一键数异常: {len(set(keys))} (期望 494; V6.9.3 主题共享 +6: themeHues/themeHueNames/themeHue/themeMode/hueColor/hueName; F11.2 策略研究恒显 -2: researchMenuEnabled/toggleResearchMenu; V5.16 详情双栏 +4: detailDisplayMode/setDetailDisplayMode/isNarrow/detailSplitEnabled)"
...
def test_shortcut_help_synced():
    ...
    # 面板条目数 = 14（V5.3.3.2 扩展: +Ctrl+D/E/G/F5/Ctrl+B/Ctrl+J; V6.7.1: +Ctrl+H/Ctrl+Shift+S）
    m = re.search(r"const shortcutHelpItems = \[([\s\S]*?)\];", src)
    assert m, "shortcutHelpItems 未找到"
    count = len(re.findall(r"\{ keys:", m.group(1)))
    assert count == 14, f"帮助面板应含 14 条快捷键 (V6.7.1)，当前 {count} 条"
...
def test_lazy_research_subpage_guard():
    ...
    """
    watch = _read("js/app-logic/watch.js")
    assert "__lazyLoaders['research']" in watch, "watch 应有 research chunk 补加载"
    assert "lazyTick" in watch, "补加载后应 tick 强制 pageComp 重算"
    assert "lazyTick.value++" in watch, "补加载完成后应递增 lazyTick"
    app = _read("js/app-logic.js")
    assert "const lazyTick = ref(0);" in app, "app-logic 应有 lazyTick 定义"
    assert "void lazyTick.value" in app, "pageComp 应依赖 lazyTick"
